...
from helpers.datagenerator_emb import DataGenerator
    ...
    for iteration in xrange(start_epoch,epochs+1):
        ...
        for iter_d in xrange(iteration_d):
            ...
            for training_step in range(training_steps):
                ...
                # train with motion
                pred_v, pred_i = torch.topk(netG(real_motion_v, real_claim_v, 0.0), 1, dim=2)
                fake = autograd.Variable(torch.squeeze(pred_i).data)
                inputv = fake
        ...
        for training_step in range(num_train_data//batch_size):
            ...
            # fake = netG(real_motion_v, real_claim_v)
            fake_v, fake_i = torch.topk(netG(real_motion_v, real_claim_v), 1, dim=2)
            fake = autograd.Variable(torch.squeeze(fake_i).data)
            print fake
            f_fake, G = netD(fake)
