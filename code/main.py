...
def save_pretrain_D_checkpoint(state, filename='last_checkpoint_vanilla_pretrain_encdec_Disc.pth.tar'):
    if os.path.isfile(filename):
        os.remove(filename)
    torch.save(state, filename)

def save_pretrain_G_checkpoint(state, filename='last_checkpoint_vanilla_pretrain_encdec_Gen.pth.tar'):
    if os.path.isfile(filename):
        os.remove(filename)
    torch.save(state, filename)
...
def pretrain(run_name, netD, netG, motion_length, claim_length, embedding_dim, hidden_dim_G, hidden_dim_D, lam, batch_size, epochs, num_words, train_data_dir, test_data_dir):
    ...
    for iteration in xrange(start_epoch,epochs+1):
        ...
        if iteration % 100 == 0:
            result_text = open("pretraining_generated/pretrain_" + datetime.datetime.now().strftime("%Y-%m-%d:%H:%M:%S") + ".txt", "w")
            testing_steps = num_test_data//batch_size
...
if __name__ == '__main__':
    ...
    if not os.path.isfile(os.path.join(processed_data_dir, 'lexicon-dict.pkl')):
        ...
        netG = GeneratorEncDec(batch_size, lexicon_count, motion_length, claim_length, hidden_dim_G, embedding_dim)
