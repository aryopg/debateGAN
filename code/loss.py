...
class JSDLoss(nn.Module):
    ...
    def forward(self, batch_size, f_real, f_synt):
        ...
        dev_f_real = f_real - f_real_mean
        dev_f_synt = f_synt - f_synt_mean

        f_real_xx = torch.mm(dev_f_real, torch.t(dev_f_real))
        f_synt_xx = torch.mm(dev_f_synt, torch.t(dev_f_synt))

        cov_mat_f_real = f_real_xx / batch_size
        cov_mat_f_synt = f_synt_xx / batch_size
...
class MMDLoss(nn.Module):
    ...
    def forward(self, batch_size, f_real, f_synt):
        ...
        x_sq = torch.cumsum(torch.mm(dist_x, cov_inv_mat) * dist_x, dim=1)
        y_sq = torch.cumsum(torch.mm(dist_y, cov_inv_mat) * dist_y, dim=1)
