...
def encode_data(motion_evidence_map, motion_claim_map, motion_article_map, lexicon_dictionary, lexicon_count):
    encoded_data = []
    lengths = []
    data_length = len(motion_evidence_map)+len(motion_claim_map)+len(motion_article_map)
    data_progress = 0

    llprint("\rEncoding Data ...! %d/%d" % (data_progress,data_length))
    ...
    for key, value in motion_evidence_map.iteritems():
        ...
        for char in value:
            if char.isalpha() or char == '?' or char == '.' or char == ' ':
                encoded_motion.append(lexicon_dictionary[char.lower()])

        encoded_sentence = []
        for char in key:
            if char.isalpha() or char == '?' or char == '.' or char == ' ':
                encoded_sentence.append(lexicon_dictionary[char.lower()])
...
if __name__ == '__main__':
    ...
    processed_data_dir = join(task_dir, 'data_char')
