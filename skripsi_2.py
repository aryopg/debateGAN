...
inv_lexicon_dictionary = pickle.load(open(os.path.join(processed_data_dir, 'lexicon-dict-inverse.pkl'), 'rb'))
...
def train(input_variable, target_variable, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion, max_length=MAX_LENGTH):
    ...
    # decoder_output_full = autograd.Variable(torch.zeros(target_length, 1, self.vocab_size))
...
def trainIters(encoder, decoder, n_iters, print_every=1000, plot_every=2, learning_rate=0.01):
    ...
    for iter in range(1, num_train_data + 1):
        ...
        if iter % print_every == 0:
            ...
            print_loss_total = 0
            print('%s (%d %d%%) %.4f' % (timeSince(start, iter / num_train_data),
                                         iter, iter / num_train_data * 100, print_loss_avg))
...
def evaluate(encoder, decoder, sent, max_length=MAX_LENGTH):
    # print sentence
    ...
    for di in range(max_length):
        ...
        if ni == EOS_token:
            ...
            decoded_words.append(inv_lexicon_dictionary[ni])
...
def decode(sentence):
    ret = []
    for idx in sentence:
        ret.append(inv_lexicon_dictionary[idx])
    return " ".join(ret)

def evaluateRandomly(encoder, decoder, n=10):
    for i in range(n):
        motion, claim = DataGenerator(lexicon_count=lexicon_count, motion_length=motion_length, claim_length=claim_length, num_data=num_test_data, data_dir=test_data_dir, batch_size=1, shuffle=True).generate().next()
        # pair = random.choice((motions, claims))
        # print pair
        # print(motion)
        motion_sent = str(decode(motion[0]))
        claim_sent = decode(claim[0])
        print('>', motion_sent)
        print('=', claim_sent)
        output_words, attentions = evaluate(encoder, decoder, motion_sent)
...
evaluateAndShowAttention("this house believes that the sale of violent video games to minors should be banned")

evaluateAndShowAttention("this house supports the one-child policy of the republic of China".lower())
